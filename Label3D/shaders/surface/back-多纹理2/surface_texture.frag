...
void main() {
	...
	int map_diffuse = DataIn.texId;

#ifndef ENABLE_ALPHA
    vec3 color = texture(textureID[map_diffuse], DataIn.texcoord * repeat).rgb;
    float alpha = 1.0f;
#else
    vec4 tmp = texture(textureID[map_diffuse], DataIn.texcoord * repeat);
    if (tmp.a <= 0)
        discard;
    vec3 color = tmp.rgb;
    float alpha = tmp.a;
