...
uniform sampler2D textureID[10];
...
void main() {
	...
	highp int map_diffuse = int(DataIn.texcoord.z);

#ifndef ENABLE_ALPHA
    vec3 color = texture(textureID[map_diffuse], vec2(DataIn.texcoord.x, DataIn.texcoord.y) * repeat).rgb;
    float alpha = 1.0f;
#else
    vec4 tmp = texture(textureID[map_diffuse], vec2(DataIn.texcoord.x, DataIn.texcoord.y) * repeat);
    if (tmp.a <= 0)
        discard;
    vec3 color = tmp.rgb;
    float alpha = tmp.a;
