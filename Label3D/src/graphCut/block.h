...
template <class Type> class GraphBlock
{
public:
	...
	GraphBlock(int size, void (*err_function)(char *) = NULL) { first = last = NULL; block_size = size; error_function = err_function; }

	/* Destructor. Deallocates all items added so far */
	~GraphBlock() { while (first) { block *next = first -> next; delete[] ((char*)first); first = next; } }
