...
glmFindOrAddTexture(GLMmodel* model, const char* name)
{
    ...
    /*model->textures[model->numtextures-1].id =
        glmLoadTexture(filename, GL_TRUE, GL_TRUE, GL_TRUE, GL_FALSE, &width, &height);*/
...
GLuint findEdge(GLMedge *edges, GLuint numedges, GLuint v0, GLuint v1)
{
	GLuint i;
	GLuint vmin = v0<v1? v0:v1;
	GLuint vmax = v0<v1? v1:v0;
	for (i = 0; i < numedges; i++) 
	{
		GLuint *vidx = edges[i].vindices;
		if(vidx[0]==vmin && vidx[1]==vmax)
			return i;
	}
	return numedges;
}

GLvoid glmBuildEdge(GLMmodel* model)
{
	GLMedge *edgesRedundant  = (GLMedge*)malloc(sizeof(GLMedge) *
									model->numtriangles *3);
	int numedges = 0;
	...
	while (group) 
	{
	    for (i = 0; i < group->numtriangles; i++) 
		{
			...
			assert(triangle->vindices[0]>=1 && triangle->vindices[0]<=model->numvertices);
			assert(triangle->vindices[1]>=1 && triangle->vindices[1]<=model->numvertices);
			assert(triangle->vindices[2]>=1 && triangle->vindices[2]<=model->numvertices);
			for (j=0; j<3; j++) 
			{
				GLuint v0 = triangle->vindices[j];
				GLuint v1 = triangle->vindices[(j+1)>2?0:(j+1)];
				GLuint idx = findEdge(edgesRedundant,numedges,v0,v1);	// 改成以点来检索，提高效率
				if(idx==numedges) // not existed
				{					
					edgesRedundant[numedges].vindices[0] = v0<v1 ? v0:v1;
					edgesRedundant[numedges].vindices[1] = v0<v1 ? v1:v0;
					edgesRedundant[numedges].findices[0] = group->triangles[i];
					model->triangles[(group->triangles[i])].eindices[j] = numedges;
					numedges++;
				}
				else // existed in idx
				{					
					edgesRedundant[idx].findices[1] = group->triangles[i];
					model->triangles[(group->triangles[i])].eindices[j] = idx;
				}
		    }
	    }	    
	    group = group->next;
	...
	// save edge datas
	model->numedges = numedges;
	model->edges  = (GLMedge*)malloc(sizeof(GLMedge) * model->numedges);
	memcpy(model->edges,edgesRedundant,sizeof(GLMedge) * model->numedges);

	free(edgesRedundant);
}

GLvoid glmComputeFaceAdjacent(GLMmodel* model)
{
	//int i, j, idx;
	//int lenDefault = 5;
	//GLuint **vertex2Tri  = (GLuint**)malloc(sizeof(GLuint*) * model->numvertices);
	//for(i=0; i<model->numvertices; i++)
	//	vertex2Tri[i] = (GLuint*)malloc(sizeof(GLuint) * lenDefault);
	//GLuint *vertexNum =  (GLuint*)malloc(sizeof(GLuint) * model->numvertices);
	//memset(vertexNum,0,sizeof(GLuint) * model->numvertices);

	//// getting vertex dictionary
	//GLMgroup* group = model->groups;
	//GLMtriangle* triangle;
	//while (group) 
	//{
	...
	//		for (j=0; j<3; j++) 
	//		{
	//			idx = triangle->vindices[j];
	//			assert(idx>=1 && idx<=model->numvertices);
	//			if( vertexNum[idx] < lenDefault )
	//			{
	//				vertex2Tri[idx][vertexNum[idx]] = group->triangles[i];
	//				vertexNum[idx]++;
	//			}
	//			else
	//			{
	//				// todo : 当某个顶点相邻的三角面片超过lenDefault时，需要增加内存
	//			}
	//		}
	//	}
	//}
...
#ifdef EDGE_SUPPORTED
	glmBuildEdge(model);
#endif

#ifdef FACE_ADJACENT
	glmComputeFaceAdjacent(model);
#endif
 ...
 *             GLM_POINT    -  render with only vertices
...
glmWriteOBJ(GLMmodel* model, char* filename, GLuint mode)
{
    GLuint  i;
    FILE*   file;
    GLMgroup* group;    GLuint material = -1;
 ...
 *             GLM_POINT    -  render with only vertices
 *			   GLM_LINE     -  render with only lines
 *             GLM_FLAT     -  render with facet normals
 *             GLM_SMOOTH   -  render with vertex normals
 *             GLM_TEXTURE  -  render with texture coords
 *             GLM_COLOR    -  render with colors (color material)
...
//add by xyh 2019-7-3
GLvoid
glmComputPosition(GLMmodel* model)
{
	float minx = 99999999999, miny = 99999999999, minz = 99999999999;
	float maxx = -99999999999, maxy = -99999999999, maxz = -99999999999;

	for (int i = 1; i <= model->numvertices; i++)
	{
		minx = minx <= model->vertices[3 * i + 0] ? minx : model->vertices[3 * i + 0];
		maxx = maxx >= model->vertices[3 * i + 0] ? maxx : model->vertices[3 * i + 0];
		miny = miny <= model->vertices[3 * i + 1] ? miny : model->vertices[3 * i + 1];
		maxy = maxy >= model->vertices[3 * i + 1] ? maxy : model->vertices[3 * i + 1];
		minz = minz <= model->vertices[3 * i + 2] ? minz : model->vertices[3 * i + 2];
		maxz = maxz >= model->vertices[3 * i + 2] ? maxz : model->vertices[3 * i + 2];
	}

	model->position[0] = (minx + maxx) / 2.0;
	model->position[1] = (miny + maxy) / 2.0;
	model->position[2] = (minz + maxz) / 2.0;

	model->widthLengthHeight[0] = maxx - minx;
	model->widthLengthHeight[1] = maxy - miny;
	model->widthLengthHeight[2] = maxz - minz;
}
