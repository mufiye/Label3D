...
//#define EDGE_SUPPORTED
//#define FACE_ADJACENT

#ifndef M_PI
#define M_PI 3.14159265f
#endif

#define GLM_POINT    (0)            /* render with only vertices */
#define GLM_LINE    (1 << 0)            /* render with only vertices */
...
#define GLM_2_SIDED  (1 << 6)       /* render two-sided polygons */
...
typedef struct _GLMtriangle {
  ...
  GLuint eindices[3];           /* array of triangle edge indices*/
  GLuint findices[3];			/* array of adjacent face indeices */
...
typedef struct _GLMedge {
  GLuint vindices[2];           /* array of triangle vertex indices */
  GLuint findices[2];           /* array of triangle facet indices */
} GLMedge;
...
typedef struct _GLMmodel {
  ...
  GLfloat* vertices;            /* array of vertices  索引从1开始：[1, numvertices]；第i个顶点xyz坐标：(vertices[3*i+0], vertices[3*i+1], vertices[3*i+2]) */
  ...
  GLuint       numedges;    /* number of triangles in model */
  GLMedge*	   edges;       /* array of triangles */
  ...
  GLfloat widthLengthHeight[3];          /*width（x） length(y) and height(z) of the model */
