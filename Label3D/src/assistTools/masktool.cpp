...
void masktool::rleDecode(const RLE *R, byte *M, siz n) {
	for (siz i = 0; i<n; i++)
	{
		byte v = 0;
		for (siz j = 0; j < R[i].m; j++)
		{
			for (siz k = 0; k < R[i].cnts[j]; k++)
				*(M++) = v;
			v = !v;
		}
	}
...
void masktool::rleMerge(const RLE *R, RLE *M, siz n, bool intersect) {
...
void masktool::rleArea(const RLE *R, siz n, uint *a) {
	for (siz i = 0; i<n; i++) {
		a[i] = 0; for (siz j = 1; j<R[i].m; j += 2) a[i] += R[i].cnts[j];
	}
}

void masktool::rleIou(RLE *dt, RLE *gt, siz m, siz n, byte *iscrowd, double *o) {
...
void masktool::bbIou(BB dt, BB gt, siz m, siz n, byte *iscrowd, double *o) {
...
void masktool::rleToBbox(const RLE *R, BB bb, siz n) {
	for (siz i = 0; i<n; i++) {
...
void masktool::rleFrBbox(RLE *R, const BB bb, siz h, siz w, siz n) {
	for (siz i = 0; i<n; i++) {
...
void masktool::rleFrPoly(RLE *R, const double *xy, siz k, siz h, siz w) {
...
char* masktool::rleToString(const RLE *R) {
...
void masktool::rleFrString(RLE *R, char *s, siz h, siz w) {
...
//读取文件中Mask信息
vector<SMask> masktool::ReadMasks(const string &fileName)
{
	vector<string> imageList;
	vector<SMask> maskList;
	...
	if (ok)
	{
		...
		for (int i = 0; i < imageSize; i++)
		{
			...
			string photoName = root["images"][i]["file_name"].asString();
			imageList[id - 1] = photoName;
		}
		...
		for (int i = 0; i < maskCount; i++)
		{
			...
			sr.imageName = imageList[sr.imageId - 1];
...
//解码Mask区域为长度h*w的byte数组
byte* masktool::DecodeMask(const string& counts, int w, int h)
{
	RLE* rle = new RLE();
	char* s = (char*)counts.c_str();
	rleFrString(rle, s, h, w);
	byte* mask = (byte*)malloc(sizeof(byte)*rle->h*rle->w);
	rleDecode(rle, mask, 1);
	byte* reshapeMask = (byte*)malloc(sizeof(byte)*rle->h*rle->w);
	for (int row = 0; row < rle->h; row++)
	{
		for (int col = 0; col < rle->w; col++)
		{
			reshapeMask[row*(rle->w) + col] = mask[col*(rle->h) + row];
		}
	}
	free(mask);
	return reshapeMask;
